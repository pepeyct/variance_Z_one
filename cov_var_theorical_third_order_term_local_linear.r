...
calculus_cal = foreach(i = 1:50,.combine = 'rbind',.packages = c("dplyr")) %dopar% {
  ...
  grid_triple_point = grid_triple_point %>% mutate(a11=w_h_1*w_h_2*lam_prod*pcf(dis1)*pcf(dis2),a12=a11*(dis2-r)/h,a21=a11*(dis1-r)/h,a22=a11*((dis2-r)*(dis1-r))/(h^2))
  C_vec= colSums(grid_triple_point[,10:13])*bin_grid_c^3
  C_vec
}
proc.time()-ptm
# three order calculus
save(calculus_cal,file = "cov_var_calculus_localconstant_feb12.RData")
