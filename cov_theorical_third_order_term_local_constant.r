...
calculus_cal = foreach(i = 1:50,.combine = 'rbind',.packages = c("dplyr")) %dopar% {
  ...
  grid_triple_point = grid_triple_point %>% mutate(final_prod=w_h_1*w_h_2*lam_prod*pcf(dis1)*pcf(dis2))
  C_value= sum(grid_triple_point$final_prod)*bin_grid_c^3
  C_value
}
proc.time()-ptm
# three order calculus
save(calculus_cal,file = "cov_calculus_localconstant_feb12.RData")
